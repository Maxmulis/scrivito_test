...
p.update(valid_from: Time.current - 1.day, valid_until: Time.current + 1.day, _path: "/news/#{p.id}")
...
p.update(valid_from: Time.current - 2.days, valid_until: Time.current - 1.day, _path: "/news/#{p.id}")
puts "Created expired page"
puts "Path: #{p.path}"

# Create a future page
p = NewsPage.create(title: "Future")
p.update(valid_from: Time.current + 1.day, valid_until: Time.current + 2.days, _path: "/news/#{p.id}")
puts "Created future page"

# Create a page with no valid_from
p = NewsPage.create(title: "No valid_from")
p.update(valid_until: Time.current + 2.days, valid_from: nil, _path: "/news/#{p.id}")
puts "Created no valid_from page"

# Create a page with no valid_until
p = NewsPage.create(title: "No valid_until")
p.update(valid_from: Time.current - 2.days, valid_until: nil, _path: "/news/#{p.id}")
